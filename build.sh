...
if [[ "${TARGET}" == win* ]]; then
    ...
    docker run --rm -i $TTY_ARG "${UIDARGS[@]}" -v "${ARTIFACTS_PATH}":/out -v "${PWD}/${BUILD_NAME}":"/${BUILD_NAME}" -w / "$IMAGE" ZSTD_CLEVEL=19 ZSTD_NBTHREADS=0 tar -cf --zstd "/out/${OUTPUT_FNAME}" "$BUILD_NAME"
fi
cd -
